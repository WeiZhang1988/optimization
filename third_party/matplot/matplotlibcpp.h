...
//
// @mainpage https://matplotlib-cpp.readthedocs.io/en/latest/docs.html
   ...
   private:
    ...
    _interpreter() {
        ...
        if (!s_python_function_show || !s_python_function_close || !s_python_function_draw ||
            ...
            !s_python_function_fignum_exists || !s_python_function_plot ||
            !s_python_function_quiver || !s_python_function_contour ||
            !s_python_function_contourf || !s_python_function_colorbar ||
            ...
            !s_python_function_legend || !s_python_function_ylim || !s_python_function_title ||
            !s_python_function_axis || !s_python_function_xlabel || !s_python_function_ylabel ||
            !s_python_function_xticks || !s_python_function_yticks || !s_python_function_xscale ||
            !s_python_function_yscale || !s_python_function_grid || !s_python_function_xlim ||
            !s_python_function_ion || !s_python_function_ginput || !s_python_function_save ||
            ...
            !s_python_function_stem || !s_python_function_xkcd || !s_python_function_text ||
...
bool plot_base(PyObject *const pyfunc, const VectorX &x, const VectorY &y,
               const std::string &s = "", const std::map<std::string, std::string> &keywords = {}) {
...
bool loglog(const VectorX &x, const VectorY &y, const std::string &s = "",
            const std::map<std::string, std::string> &keywords = {}) {
    return detail::plot_base(detail::_interpreter::get().s_python_function_loglog, x, y, s,
                             keywords);
}
...
bool semilogx(const VectorX &x, const VectorY &y, const std::string &s = "",
              const std::map<std::string, std::string> &keywords = {}) {
    return detail::plot_base(detail::_interpreter::get().s_python_function_semilogx, x, y, s,
                             keywords);
}
...
bool semilogy(const VectorX &x, const VectorY &y, const std::string &s = "",
              const std::map<std::string, std::string> &keywords = {}) {
    return detail::plot_base(detail::_interpreter::get().s_python_function_semilogy, x, y, s,
                             keywords);
}
...
template <typename Matrix>
void levels_contour(const Matrix &x, const Matrix &y, const Matrix &z, long levels = 10,
                    const std::map<std::string, std::string> &keywords = {}) {
    detail::_interpreter::get();
    ...
    PyDict_SetItemString(kwargs, "cmap", python_colormap_coolwarm);
    PyDict_SetItemString(kwargs, "levels", PyInt_FromLong(levels));
...
void contourf(const Matrix &x, const Matrix &y, const Matrix &z,
    ...
    PyObject *res =
        PyObject_Call(detail::_interpreter::get().s_python_function_contourf, args, kwargs);
    if (!res) throw std::runtime_error("failed surface");
...
template <typename Matrix>
void levels_contourf(const Matrix &x, const Matrix &y, const Matrix &z, long levels = 10,
                     const std::map<std::string, std::string> &keywords = {}) {
    detail::_interpreter::get();
...
bool hist(const VectorY &y, long bins = 10, std::string color = "b", double alpha = 1.0,
